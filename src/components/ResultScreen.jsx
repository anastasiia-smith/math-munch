...
function ResultScreen({
  ...
  const getAmountOfCoins = () => {
    if (difficulty === 'simple') return 1;
  ...
  return (
    <section className='game-card result-screen'>
      ...
      <p className='result-text'>
        ...
        <strong>{totalQuestions}</strong> right!
      </p>
      {earnedCoin ? (
        <p className='perfect-note'>{`Perfect 5/5! You earned ${getAmountOfCoins} Om Nom coin${
          difficulty !== 'simple' ? 's' : ''
        }.`}</p>
      ) : (
        <p className='result-text small-note'>
          Keep going to collect more coins!
        </p>
      )}
        ...
        <section className='review-box' aria-label='missed questions review'>
          ...
          <ul className='review-list'>
            {wrongAnswers.map((item, index) => (
              <li
                key={`${item.first}-${item.operation}-${item.second}-${index}`}
              >
                {item.first} {item.operation} {item.second} ={' '}
                {item.correctAnswer}
                <span className='review-note'>
                  {' '}
                  (you answered {item.selectedAnswer})
                </span>
              </li>
            ))}
          </ul>
